...
class WebCompatibleAnalyzer:
    ...
    def calculate_rotation_metrics(self, data_points, analysis, total_start, graph_width, graph_info=None, ocr_data=None, game_type='パチンコ'):
        ...
        Returns:
            dict: 回転率メトリクス
        """
        log(f"[DEBUG] calculate_rotation_metrics called: total_start={total_start}, graph_width={graph_width}, has_graph_info={graph_info is not None}, has_data_points={data_points is not None and len(data_points) if data_points else 0}")
        if graph_info:
            log(f"[DEBUG] graph_info details: start_x={graph_info.get('start_x')}, end_x={graph_info.get('end_x')}")
        ...
        try:
            ...
            if graph_info and graph_info.get('start_x') is not None and graph_info.get('end_x') is not None:
                ...
                log(f"[spins_per_pixel計算] graph_info使用: start_x={graph_info['start_x']}px, end_x={graph_info['end_x']}px, width={actual_graph_width}px")
            else:
                # グラフ座標情報がない場合は従来の方法
                actual_graph_width = graph_width
                graph_start_x = 0
                log(f"[spins_per_pixel計算] graph_widthを使用: {graph_width}px")

            # 1ピクセルあたりの回転数（実際のグラフ幅を使用）
            spins_per_pixel = total_spins / actual_graph_width if actual_graph_width > 0 else 0
            log(f"[spins_per_pixel計算] total_spins={total_spins}, width={actual_graph_width}px, result={spins_per_pixel}")
            log(f"[spins_per_pixel計算] 仮定: グラフ幅({actual_graph_width}px)=全回転数({total_spins})")
            ...
            if len(data_points) > 0:
                ...
                log(f"[累計スタート計算] end_x={end_x}, graph_start_x={graph_start_x}, relative_end_x={relative_end_x}, spins_per_pixel={spins_per_pixel}, cumulative_total_spins={cumulative_total_spins}")
            else:
