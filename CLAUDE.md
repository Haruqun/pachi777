...
### 不要な領域（読み込み不要）
- **グラフ下部の小さな日付** (y座標: 1482-1509付近)
  - 例: bbox(710, 1482, 757, 1509)の「8/6」
  - 例: bbox(1093, 1482, 1142, 1509)の「8/5」
  - これらは小グラフの日付表示で、データとしては不要
...
### 禁止事項
- **Streamlitアプリを起動しない** - コードを作成したらプッシュするだけ
- **ローカルでテストを実行しない** - デプロイ環境でテストする
- **自動的にサービスを起動しない** - ユーザーが明示的に要求した場合のみ
- **新しいOCRテストファイルを作成しない** - 既存のファイルを編集する

### 推奨作業フロー
1. コードを作成・修正
2. `git add -A && git commit -m "説明的なコミットメッセージ" && git push`
3. ユーザーにプッシュ完了を報告
4. デプロイ環境でのテストをユーザーに委ねる

### 重要なファイル規約
- **出玉詳細のOCR編集**: 必ず `web_app/test_detail_ocr_app.py` を使用
- 新しいOCRテストファイルを作成せず、既存ファイルを更新すること
...
3. **現在回転数と通常回転数の表示バグ**
   - Claude APIの`current_rotations`が通常回転数欄に入っている
   - マッピングの修正が必要

4. **セッションステート管理**
   - ファイル削除後の再アップロードで古いデータが残る問題
   - `edited_df`と`temp_df`のクリア処理追加

詳細は `修正計画_20250901.md` を参照
