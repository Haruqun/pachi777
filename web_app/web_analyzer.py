...
class WebCompatibleAnalyzer:
    ...
    def calculate_rotation_metrics(self, data_points, analysis, total_start, graph_width):
        ...
        try:
            ...
            # 通常時（下降区間）の回転率計算 - シンプルな方法
            rotation_rate_2 = 0
            normal_decline_spins = 0
            normal_decline_balls = 0
            
            # より簡単な計算方法：最低値までの使用球数と回転数から計算
            if analysis['min_index'] > 0 and analysis['min_index'] < len(data_points):
                # 最低値のx座標（ピクセル位置）
                min_x = data_points[analysis['min_index']][0]
                # 最低値までの回転数
                total_spins_to_min = int(min_x * spins_per_pixel)
                # 最低値（使用球数の総計）
                total_balls_used = abs(analysis['min_value'])
                
                # 初当たりまでの分を除外して通常時のみを計算
                if first_hit_spins > 0 and first_hit_balls > 0:
                    normal_decline_spins = total_spins_to_min - first_hit_spins
                    normal_decline_balls = total_balls_used - first_hit_balls
                else:
                    normal_decline_spins = total_spins_to_min
