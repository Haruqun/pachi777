...
スタート関連の項目の違い（重要）：
- 「スタート」：画像の中央付近にある項目。前日最終から現在までの回転数
- 「初回特賞スタート」：画像の左下付近にある完全に別の項目。初回の大当りまでの回転数
- これらは異なる位置にある異なる項目なので、同じ値になることはありません
- 必ず別々の場所から別々の値を読み取ってください

{
  ...
  "スタート情報": {
    ...
    "通常": number,
    "チャンス中": number,
    "スタート": number（画像中央付近の「スタート」という項目の直下の数値。前日最終から現在までの回転数）,
    "初回特賞スタート": number（画像左下付近の「初回特賞スタート」という項目の直下の数値。初回大当りまでの回転数）,
...
1. 初回特賞スタートの値を必ず読み取ってください
   - 「初回特賞スタート」は画像の左下付近にあります
   - この項目名の真下にある数値（通常100〜200程度）を読み取ってください
   - 値が0になることはほぼありません
   - 左下エリアを注意深く確認してください
