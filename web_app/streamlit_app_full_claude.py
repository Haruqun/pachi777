#!/usr/bin/env python3
"""
AI Graph Analysis Report - Claude Edition
...
# ページ設定
st.set_page_config(
    page_title="AI Graph Analysis Report - Claude Edition",
    page_icon="🎰",
    layout="wide"
)
...
def detect_black_frame_and_crop(image, crop_ratio=50):
    ...
    if w > img_w * 0.5 and h > img_h * 0.5:
        # 黒枠の上部50%の位置に線を引き、画像の一番上からその線までを切り取り
        crop_line_y = y + int(h * (crop_ratio / 100))
        cropped_image = image.crop((0, 0, image.size[0], crop_line_y))
...
3. 「初回特賞スタート」（画像の左下エリア）
   - 位置：画像の左下付近にある独立した項目
   - 意味：初回の大当りまでに要した回転数
   - 数値：通常50〜500回の範囲（初当たりがない場合は0）
   - 例：「初回特賞スタート 187」のような表記

これら3つは完全に異なる数値です。位置と数値の大きさから正確に識別してください。
累計スタート > スタート ≥ 初回特賞スタート の関係が一般的です。

{
  ...
  "スタート情報": {
    "累計スタート": number（画像上部にある最も大きいスタート数値。通常1000〜10000の範囲）,
    "通常": number,
    "チャンス中": number,
    "スタート": number（画像中央付近の「スタート」項目の数値。当日の回転数。通常0〜2000の範囲）,
    "初回特賞スタート": number（画像左下の独立した「初回特賞スタート」項目の数値。初当たりまでの回転数。通常50〜500の範囲）,
...
重要な注意事項：
1. スタート関連の3つの項目を必ず区別してください
   - 累計スタート：画像上部の最大値（1000〜10000）
   - スタート：画像中央の当日回転数（0〜2000）
   - 初回特賞スタート：画像左下の初当たり回転数（50〜500、初当たりなしなら0）
   - これらは絶対に混同しないでください

2. 初回特賞スタートの値を正確に読み取ってください
   - 「初回特賞スタート」は画像の左下付近の独立した項目です
   - 累計スタートやスタートとは完全に別の項目です
   - この項目名の真下にある数値を読み取ってください
   - 通常50〜500程度の値です（初当たりがあった場合）

3. ラウンド情報は慎重に読み取ってください
...
# 本番解析セクション
st.markdown("---")
st.markdown("## 🎰 AI Graph Analysis Report - Claude Edition")
st.caption("""高精度データ抽出・解析システム with Claude API - Professional Edition
...
with col1:
    st.caption("**グラフ画像（必須）** - site7のグラフ画像を選択してください")
...
with col2:
    st.caption("**出玉詳細画像（オプション）** - Claude APIで詳細データを読み取ります")
    detail_files = st.file_uploader(
        "詳細画像を選択",
        type=['jpg', 'jpeg', 'png'],
        accept_multiple_files=True,
        help="出玉詳細画像をアップロードするとより詳細なデータが取得できます",
        key="detail_uploader"
    )
    ...
    for idx, uploaded_file in enumerate(uploaded_files):
        ...
        if graph_data_points:
            ...
            analysis_results.append({
                ...
                'ocr_data': ocr_data,  # OCRデータを追加（Claude APIデータも統合済み）
                'ocr_text': ocr_data.get('ocr_text') if ocr_data else None,  # OCRテキストを追加
                'claude_data': claude_data,  # Claude APIデータを個別に保存
