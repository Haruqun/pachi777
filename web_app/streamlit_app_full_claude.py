...
■ 領域B：下部の白背景テーブル（2行×5列）
  - ここには「最高一撃獲得」などがありますが、読み取り不要です
  - ただし「初回特賞スタート」は下部テーブルの左下にあるので読み取ってください

【読み取り手順】
1. まず右上の黒枠内から「累計スタート」「通常」を読む
2. 下部テーブルから「初回特賞スタート」のみを読む
3. 「最高一撃獲得」などの不要な項目は無視する

【特に注意】
1. 累計スタートの位置を間違えないでください
   - 累計スタートは必ず右上の黒枠内にあります（領域A）
   - 下部テーブルの「最高一撃獲得」を累計スタートと誤読しないでください
   - 「累計スタート」というラベルのすぐ下の数値のみを読んでください

2. 通常の位置を確認してください
   - 通常は右上の黒枠内の左下にあります（領域A）
   - 「通常」というラベルの直下の数値を読んでください

3. 初回特賞スタートの位置
   - 「初回特賞スタート」は下部テーブルの2段目の左端にあります（領域B）
   - 「初回特賞スタート」というラベルの真下の数値を読んでください

4. 初当り回数と大当り回数
   - これらは画像の上部中央付近にあります
   - 「初当り回数」と「大当り回数」のラベルを確認して読んでください

5. ラウンド内訳（超・中・小）
   - 画像の中央左付近にあります
   - 「超」「中」「小」それぞれの文字の真下の数値を読んでください
