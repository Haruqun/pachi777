...
1. スタート関連の項目を必ず区別してください
   - 累計スタート：画像上部の最大値（必ず最も大きい数値）
   - 通常：累計スタートの直下（累計スタートの60-95%程度の値）
   - スタート：画像中央の当日回転数（累計スタートより小さい）
   - 初回特賞スタート：画像左下の初当たり回転数（初当たりがない場合は0）
   - これらは位置で区別し、絶対に混同しないでください
   - 特に「通常」は累計スタートより必ず小さくなければならない
