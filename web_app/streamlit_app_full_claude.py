...
1. スタート関連の項目を必ず区別してください
   - 累計スタート：「累計スタート」というテキストの直下（数十ピクセル以内）にある数値のみ
   - 通常：「通常」というテキストの直下（数十ピクセル以内）にある数値のみ
   - 初回特賞スタート：「初回特賞スタート」というテキストの直下（数十ピクセル以内）にある数値のみ
   - 離れた位置にある数値は絶対に読み取らないでください
   - テキストラベルと数値は必ず上下に近接している必要があります

2. 初回特賞スタートと初当り回数を混同しないでください
   - 「初回特賞スタート」は画像の左下付近の独立した項目です
   - 「初回特賞スタート」というテキストラベルの真下にある数値のみが初回特賞スタートです
   - 「初当り回数」は別の場所にある完全に異なる項目です
   - 初回特賞スタートと初当り回数の数値を入れ替えないでください
   - 各項目は必ずそのテキストラベルの直下の数値を読み取ってください

3. ラウンド情報は慎重に読み取ってください
   - 「超」「中」「小」それぞれの文字の真下の数値を読み取ってください
   - 各項目の真下にある数値を正確に読み取ってください

4. 最高出玉と累計スタートを混同しないでください
